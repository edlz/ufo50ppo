...
/// Returns (reward, done, clean). If clean is false, caller should not update prev.
pub type RewardFn =
    fn(prev: Option<&[u8]>, curr: &[u8], width: u32, height: u32) -> (f64, bool, bool);

/// Stub reward — always returns 0 and never terminates.
pub fn stub_reward(_prev: Option<&[u8]>, _curr: &[u8], _w: u32, _h: u32) -> (f64, bool, bool) {
    (0.0, false, true)
}
...
impl GameEnv {
    ...
    pub fn step(&mut self, action: usize) -> (Tensor, f64, bool) {
        ...
        let frame = self.frame_rx.recv().expect("capture thread died");
        let (reward, done, clean) = (self.reward_fn)(
