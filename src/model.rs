...
impl ActorCritic {
    pub fn new(device: Device) -> Self {
        ...
        let shared = nn::seq()
            .add(nn::conv2d(root / "c1", 4, 32, 8, nn::ConvConfig { stride: 4, ..Default::default() }))
            .add_fn(|x| x.relu())
            .add(nn::conv2d(root / "c2", 32, 64, 4, nn::ConvConfig { stride: 2, ..Default::default() }))
            .add_fn(|x| x.relu())
            .add(nn::conv2d(root / "c3", 64, 64, 3, nn::ConvConfig { stride: 1, ..Default::default() }))
            .add_fn(|x| x.relu())
    ...
    pub fn forward(&self, obs: &Tensor) -> (Tensor, Tensor) {
        ...
        let log_probs = self.policy_head.forward(&features).log_softmax(-1, Kind::Float);
